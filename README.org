...
This plugin implements a simple interface that randomly displays wells from a grid of images and gets and records a user-entered score. It also generates a simple HTML report that matches the images to the score to allow a user to check their scoring. The plugin requires the ImageJ plugin =Microarray Profile= to generate a grid of coordinates for the images that are cropped and displayed.
